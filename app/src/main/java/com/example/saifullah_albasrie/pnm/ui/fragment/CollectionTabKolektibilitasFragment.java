 ...
 */
public class CollectionTabKolektibilitasFragment extends BaseFragment {

    private int formMode;
    private PnmDebiturModel mDataModel;
    private CollectionModel mCollectionModel;

    @BindView(R.id.tv_data_kolektibilitas)
    EditText tvDataKolektibilitas;
    @BindView(R.id.tv_hint_data_kolektibilitas)
    TextView tvHintDataKolektibilitas;

    @BindView(R.id.tv_hint_tindak_lanjut)
    TextView tvHintTindakLanjut;
    @BindView(R.id.tv_tindak_lanjut)
    EditText tvTindakLanjut;

    @Override
    public int getFragmentView() {
        return R.layout.fragment_collection_kolektibilitas;
    }
    ...
    private void setupView() {
        if (mDataModel != null) {
            tvDataKolektibilitas.setText(mDataModel.getKolektibilitas());
        }

        if (mCollectionModel != null) {
            tvTindakLanjut.setText(mCollectionModel.getTindakLanjut());
        }
    ...
    private void setEnableItems(boolean enable) {
        //always disable
        tvHintDataKolektibilitas.setEnabled(false);
        tvDataKolektibilitas.setEnabled(false);
        tvHintTindakLanjut.setEnabled(enable);
        tvTindakLanjut.setEnabled(enable);
    }
    ...
    public CollectionModel saveData() {
        ...
        model.setTindakLanjut(tvTindakLanjut.getText().toString());
        return model;
    }
}
