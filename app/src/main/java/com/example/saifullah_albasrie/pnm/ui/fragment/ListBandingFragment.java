...
import com.example.saifullah_albasrie.pnm.controller.BandingListController;
...
import com.example.saifullah_albasrie.pnm.ui.activity.BandingDetailActivity;
...
public class ListBandingFragment extends BaseFragment implements OnItemClickListener, BandingListCallback, AppBarLayout.OnOffsetChangedListener {
    ...
    private BandingListController controller;
    ...
    private void loadData(boolean clearData) {
        ...
        controller.getBandingList(page);

        //DUMMY
        /*
        listFeedback.clear();
        FeedbackItemModel model = new FeedbackItemModel();
        model.setNamaPanggilan("Debitur Satu");
        model.setModifiedDate("2017-04-20");
        model.setKeterangan("Banding dong..");
        listFeedback.add(model);
        FeedbackItemModel model2 = new FeedbackItemModel();
        model2.setNamaPanggilan("Debitur Dua");
        model2.setModifiedDate("2017-04-22");
        model2.setKeterangan("Banding lagi dong..");
        listFeedback.add(model2);
        listFeedbackCopy.clear();
    ...
    private void initData() {
        controller = new BandingListController(this);
    ...
    public void onItemClick(View itemView, View view, int position, long id) {
        BandingDetailActivity.navigate(getActivity(), listFeedback.get(position));
    }
    ...
    public void doSearch(String query) {
        ...
        if(TextUtils.isEmpty(query)) {
            ...
            if (!found) {
                tvMessage.setText(getString(R.string.banding_not_found));
                tvMessage.setVisibility(View.VISIBLE);
            } else {
                tvMessage.setVisibility(View.GONE);
            }
    ...
    @Override
    public void onGetListBandingSuccess(List<FeedbackItemModel> listItemModels) {
        DialogFactory.dismissProgress();
        if(!isVisible()) {
            return;
        }
    ...
    @Override
    public void onGetListBandingFailed(Throwable t) {
        DialogFactory.dismissProgress();
        if(!isVisible()) {
            return;
        }
