...
import com.example.saifullah_albasrie.pnm.controller.ListProspekController;
...
import com.example.saifullah_albasrie.pnm.ui.activity.FormProspekActivity;
...
public class ListProspekFragment extends BaseFragment implements OnItemClickListener, ListProspekCallback, AppBarLayout.OnOffsetChangedListener {

    public static final int NEW_PROSPEK = 200;
    ...
    private ListProspekController controller;
    ...
    private void loadData(boolean clearData) {
        ...
        controller.getProspekList(page);
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_search) {
            ...
            FilterActivity.navigate(this, FilterRequestType.FILTER_PROSPEK);
            return true;
        }
    ...
    private void initView() {
        ...
        fabAdd.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                //showSnakbarMessage(view, "Tambah Prospek");
                showAddProspek();
            }
        });
    }
    ...
    private void showAddProspek() {
        Intent intent = new Intent(getContext(), FormProspekActivity.class);
        startActivityForResult(intent, NEW_PROSPEK);
    }
    ...
    private void initData() {
        ...
        controller = new ListProspekController(this);
    }


    @Override
    public void onItemClick(View itemView, View view, int position, long id) {
        if (position >= 0 && position < listProspek.size()) {
            Intent intent = new Intent(getContext(), FormProspekActivity.class);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == FilterRequestType.FILTER_PROSPEK && resultCode == getActivity().RESULT_OK) {
        ...
        } else if (requestCode == NEW_PROSPEK && resultCode == getActivity().RESULT_OK) {
            loadData(true);
        }
    }
    ...
    @Override
    public void onGetListProspekSuccess(List<ProspekListItemModel> listItemModels) {
        DialogFactory.dismissProgress();
        if (!isVisible()) {
            return;
        }

        int oldCount = listProspek.size();
        int newCount = 0;
        if (listItemModels != null && listItemModels.size() > 0) {
            newCount = listItemModels.size();
    ...
    @Override
    public void onGetListProspekFailed(Throwable t) {
        DialogFactory.dismissProgress();
        if (!isVisible()) {
            return;
        }
